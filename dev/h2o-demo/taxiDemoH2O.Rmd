...
out <- data.frame(
  ...
  Test = c(
    ...
    R_lm = mse(~ tip_amount - pred_lm, trips_test),
...
```{r summary}
datatable(round(out[order(out$Test), ],4))
out %>%
  mutate(Model = rownames(out)) %>%
  ggplot(aes(x = reorder(Model, Train), y = Train)) + 
  geom_bar(stat = "Identity") + 
  coord_flip() +
  ggtitle("MSE by Model")
```
