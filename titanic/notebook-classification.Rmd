...
# ml_pred <- list(
#   "Logistic" = score_test_data(ml_log),
#   "Decision Tree" = score_test_data(ml_dt),
#   "Random Forest" = score_test_data(ml_rf),
#   "Gradient Boosted Trees" = score_test_data(ml_gbt),
#   "Naive Bayes" = score_test_data(ml_nb),
#   "Neural Net" = score_test_data(ml_nn)
# )
```
...
models <- list(ml_dt, ml_rf, ml_gbt, ml_nb, ml_nn)
...
auc <- function(model, data){
  p <- sdf_predict(model, data)
  auc <- sparklyr:::ml_binary_classification_eval(p, "Survived", "prediction")
  auc
}

ml_auc <- list(
  "decision tree" = auc(ml_dt, test_tbl),
  "random forest" = auc(ml_rf, test_tbl),
  "gradient boosted trees" = auc(ml_gbt, test_tbl),
  "naive bayes" = auc(ml_nb, test_tbl),
  "neural net" = auc(ml_nn, test_tbl)
)

data.frame(auc = unlist(ml_auc)) %>%
  ggplot(aes(rownames(.), auc)) +
