...
tbl_cache(sc, 'trips_par') # 3.4 minute
...
shiny_sc <<- sc
shiny_trips_tbl <<- tbl(shiny_sc, 'trips_par')
...
server <- function(input, output) {
  ...
  output$tripLeaflet <- renderLeaflet({  
      ...
      #setView(lng = -73.9, lat = 40.7, zoom = 12) %>%
      addProviderTiles("CartoDB.Positron") %>%
      addCircleMarkers(~pickup_longitude, ~pickup_latitude, fill = FALSE) %>%
      addCircleMarkers(~dropoff_longitude, ~dropoff_latitude, stroke = FALSE)
  })
