...
Killer stuff, Sean. I'd like this page to be the first one returned under the keyword search "Comparison of Spark Classifiers". You're doing more than showing off sparklyr, you're also showing off spark ml. We want python users getting routed to our pages. Wouldn't it be great if the best ml work was being done by the R community?

Document structure:

* Start with an overview that states your hypothesis and workflow
    * Overiew: This analysis is a comparison of classifiers in spark ml
    * Question: Which spark ml produces the best predictive model?
    * Considering: lm, nb, gbm, rf, cart, nn
    * Using the Titanic data
        * I don't fully understand this dataset; strange that it came pre split in train/valid and valid doesn't have labels
    * Describe workflow: Data > Impute > Train/Valid >>> Train / score 6 models >>> comparison
    * Add RMSE comparison barchart (this is important at the top so people get the answer right away)
* Break document into sections
    * Create links at top using [Section 1]
    * Use `***` horizontal lines to break sections
* Call out imputation section
    * This is optional.
    * Could be handled by removing values, imputing the mean, or breaking out another category
    * One question is: how much does imputation influence the model comparison?
    * Make sure that audiance knows (1) there are many ways to do this; (2) results may vary
    * (I wish we didn't have to do this actually as it distracts from the main message)
    * (What about not using age?)
    * Strange the residual plot isn't flat
* Models
    * Specify evaluation metrics. You should have two: AUC (is this calculated?) and MSE
    * You also have the ROC curve and confusion matrices (which are is a subset of ROC)
    * The confusion matrices might be better if they were all in the same section so they could be compared
    * Why are there two ROC curves for died/survived? Isn't that the same? Probably choose only one
    * I think most people will skip past the models unless they want to replicate your work
    * The model section should appeal to those who want to reproduce the work.
    * Structure model sections so that general readers can skip but still understand what it is doing
    * The overview and the comparison sections are the most important by far
    * Might want to have a brief discussion about interpretting the models? What are the big facators in predicting survival/death? Are they consistent across models?
* Comparions / evaluation
    * Show ROC comparison
    * Discuss AUC (http://stats.stackexchange.com/questions/145566/how-to-calculate-area-under-the-curve-auc-or-the-c-statistic-by-hand)
    * Discuss MSE (maybe table with method, auc, mse)
    * Why naive bayes sucks so bad? NB assumes conditional independence which is a fatal flaw for some classifiers
    * Speed tests? Which algos are fastest?
* Recommendations
    * as expected most predictions are comparable
    * some models run faster than others. some may be able to scale to much larger data sizes
    * some models are easier to interpret. are the effects the same? are the models telling the same story?
* Lit review (maybe appendix):
    * https://www.mapr.com/blog/comparing-kill-mockingbird-its-sequel-with-apache-spark
    * http://scikit-learn.org/stable/auto_examples/classification/plot_classifier_comparison.html
    * other comparisons of ml models? You might be the first to do this well.
