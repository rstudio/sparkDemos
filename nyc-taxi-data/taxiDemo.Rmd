...
sdf_register(model_partition_tbl$train, "trips_train")
tbl_cache(sc, "trips_train")
trips_train_tbl <- tbl(sc, "trips_train")

sdf_register(model_partition_tbl$train, "trips_test")
tbl_cache(sc, "trips_test")
trips_test_tbl <- tbl(sc, "trips_test")
  
model_formula <- formula(tip_amount ~ 
                           fare_amount + vendor_id + pickup_hour + pickup_week + 
                           passenger_count + trip_time + trip_distance)

m1 <- ml_linear_regression(train_tbl, model_formula)
summary(m1)
...
A few tips for working with Spark...

![](gizmo_small.png)

* First of all, plan time to cache data, it takes a long time to cache data, especially large amounts of data, it'll test your patience. 
* Second, don't collect too much data, not even a modest amount. 
* But the most important rule, the rule you can never forget, no matter how much you need to, no matter how much you want to, never push the stop button.
