...
# Cache table
tbl_cache(sc, "trips_par_joined") # 7 minutes
```
...
```{r}
# Calculate top geo locations for pickup and dropoff  
distinct_group <- function(data, group, cutoff = 100000){
  data %>%
    group_by_(group) %>%
    count %>%
    filter(n > cutoff) %>%
...
# Calculate top geo locations for pickup
pickup_group <<- shiny_trips_tbl %>%
  distinct_group("pickup_nta") %>%
  unlist %>%
  unname %>%
  na.omit

# Calculate top geo locations for dropoff
dropoff_group <<- shiny_trips_tbl %>%
  distinct_group("dropoff_nta") %>%
  unlist %>%
  unname %>%
  na.omit
...
ui <- miniPage(
  ...
  miniTabstripPanel(
    miniTabPanel("Inputs", icon = icon("sliders"),
                 miniContentPanel(
                   selectInput("pickup",  "Taxi origin", pickup_group, "East Village"),
                   selectInput("dropoff",  "Taxi destination", dropoff_group, "Upper West Side")
                   )
                 ),
