...
```{r kerberos, message=FALSE, warning=FALSE}
# Kerberos
system("echo '<Rstudio-password>' | kinit <Rstudio-username>")
```
...
nyct2010_tbl <- read_csv("~/sparkDemos/prod/presentations/tidyverse/nyct2010.csv") %>%
  sdf_copy_to(sc, ., "nyct2010_copy", overwrite = TRUE)
...
The linear models for Spark ML and R all had equivalent fits but different output formats. The deep learning and regression tree models had the smallest mean square errors. The mean squared errors are smaller for the training data than the test data.
